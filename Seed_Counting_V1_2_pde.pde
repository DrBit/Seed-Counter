...
void first_time_drop () {
  ...
  while (Seedcounter1.get_position() != 375) {
...
void pickup_seed_extended () {
  ...
  while (!seed_detected) {
    if (Seedcounter1.get_position() == 375) {
    ...
    if ((Seedcounter1.get_position() >= 375) || (Seedcounter1.get_position() <= 20)) {   // We chek the sensor only when we are in the range of the sensor
