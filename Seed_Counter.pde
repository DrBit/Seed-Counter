...
Stepper_ac Yaxis(22,12,5,200,4);
// pin 13 fuckt up in the arduino board
...
void first_time_drop () {
  ...
  while (Seedcounter1.get_steps() != 400) {
...
void pickup_seed_extended () {
  ...
  while (!seed_detected) {
    ...
    if ((Seedcounter1.get_steps() >= 1580) || (Seedcounter1.get_steps() <= 20)) {   // We chek the sensor only when we are in the range of the sensor
...
void go_position1 () { //arne comment: go to next position on x axle 
  motor_select = 1;
  speed_cntr_Move(6529,19000,20000,20000);
  delay (2150);
}
