...
boolean positions_updated = false;	// Variable to check if the positions accuracy has been updated already
	...
	while (!seed_detected && !skip_function()) {
		switch (counting_case) {
			...
			case case_detect_seed_position:
				...
				if (counter.get_steps() == (drop_position*counter.get_step_accuracy())) counting_case = case_drop_position; // If we are here we did not detect a seed and we already reached drop zone
			break;
...
void calibrate_counter () {
	...
	pick_seed_position = sensor_pickup;
	detect_seed_position = sensor_midpoint;
	margin_steps_to_detect_seed = sensor_margin+20;
	drop_position = sensor_release;

	positions_updated = false; 	// This is to force the update of positions, otherwise only happens once at reset
