...
void Stepper_ac::count_step(bool _temp_direction)
{
	...
	if (_stepCycle == 0) {  // If we are in the first turn we see if we gowing forward or backwards
		if (_stepPosition == (_motor_total_steps + _step_accuracy)){  // we chek the upper limit (if there was no limit (0) nothing will happen)
			_stepCycle++; 
			_stepPosition = _step_accuracy;
		}else if (_stepPosition == -(_motor_total_steps + _step_accuracy)) {  // we chek the lower limit, -1 
			_stepCycle--;
			_stepPosition = -_step_accuracy;
		}
		...
		if (_stepPosition == _motor_total_steps + _step_accuracy){  // we chek the upper limit (if there was no limit (0) nothing will happen)
			...
			_stepCycle--;
			_stepPosition = _motor_total_steps;
		}
	}else if (_stepCycle < 0) {  // Means we go backwards so count in the negative range
		if (_stepPosition == -(_motor_total_steps + _step_accuracy)){  // In negative range this is the lower limit
			_stepCycle--; 
			_stepPosition = -_step_accuracy;
